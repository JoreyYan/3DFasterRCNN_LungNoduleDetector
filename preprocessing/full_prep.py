...
#    if os.path.exists(os.path.join(prep_folder,name.split('.')[0]+'_clean.npy')):
...
#	print m1.shape,m2.shape,spacing
	...
	x,y,z = sliceim.shape
        np.save(os.path.join(prep_folder,name.split('.')[0]+'_clean'),sliceim.reshape(1,x,y,z))
        np.save(os.path.join(prep_folder,name.split('.')[0]+'_label'),np.array([[0,0,0,0]]))
    except:
